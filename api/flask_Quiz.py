...
@app.route("/spm/quiz_retrieve/<int:course_id>/<int:section>")
def get_quiz_for_learner(course_id, section):
    quiz = Quiz.query.filter_by(Course_ID=course_id, Section=section).first()
    if quiz != None:
        return jsonify(
            {
                "code": 200,
